 ...
 * Always signs out existing session first, then logs in the correct merchant
 */
...
export default function ShopifyLanding() {
  ...
  async function handleShopifyLanding() {
    ...
    // ALWAYS sign out first — we must log in as the merchant who owns THIS shop
    // not whoever was previously logged in on this browser
    await supabase.auth.signOut();

    // Look up this shop in store_installations to find the merchant email
    setStatus('Looking up your store...');
    try {
      ...
      if (installation?.shop_email) {
        // Shop is installed — send magic link to the merchant's email
    ...
    // Shop not installed yet — trigger OAuth install flow
